...
- **`mke feature` (2026-07-10, rama `feature-pods-cli`, NO mergeado) — SUCESOR de `mke dev`, mata `--env`.** Misma anatomía de pod (init clona+instala, vite HMR + tsx watch, caddy un-solo-origen, postgres efímero), ns propio `feature` en mke-preview, host `<app>-<rama>-feat.mishi.com.co` (la rama SIEMPRE en el nombre). Secretos/config resueltos por un **lease del vault** (Contrato 1, `vault-mishi`, aún NO construido — Ola 2), leyendo `mke.feature.yaml` de la app (Contrato 2: `secretos:` nombres → lease; `config:` literal → env). `up` pide `POST /v1/lease {ns,rama}` → token+leaseId inyectados (Secret `<name>-lease`, env `LEASE_TOKEN`) y etiqueta TODO el bundle `mke.feature/app|rama|lease`. `down <app> <rama>` busca el lease por esos labels → revoca (idempotente) + borra el bundle. `pull` renueva el lease. **CERO `--env` humano.** Receta pura `manifiestosFeature`/`featureName`/`featureHost` en `packages/dev-receta` (mismo paquete que `mke dev`). Cliente del vault en `cli/src/vaultLease.ts` (Bearer del emisor vía `mishi-secret get vault-mishi-emisor-token`); integración en vivo PENDIENTE (el vault no existe aún) — 70 tests unitarios cubren manifiesto/labels/lease-request/lookup-revoke con mocks. **`mke dev` queda DEPRECADO** (funcional, imprime warning apuntando a `mke feature`; no se borra — lo usan skills/músculo-memoria).
- **UN SOLO MECANISMO DE RAMA (2026-07-06, en main): `mke dev`** — pod DURADERO por app en ns `dev` de mke-preview (init clona rama + install, contenedor dev corre vite HMR + tsx watch, caddy un-solo-origen, postgres efímero; host `<app>-dev-feat`). Verbos `up/rama/pull/estado/ls/down`; `--env K=V` (Secret + envFrom; re-aplicar SIN --env los borra, y cambiar solo el env NO recicla el pod — rollout restart a mano), `--live` (vite bajo `/live/<app>/` + annotation `mke.dev/live` → Studio lo embebe same-origen), NODE_AUTH_TOKEN opcional (Secret `<name>-npm`, GitHub Packages privados, `mishi-secret get mishi-gh-read-packages-pat`). Receta pura en `packages/dev-receta`; estado en labels/annotations k8s (`mke.dev/*`) — Studio DERIVA de ahí. **Contrato `k8s/dev.env`**: archivo opcional por app con envs default; precedencia `--env` (CLI) > `k8s/dev.env` (archivo). **CANDADO (2026-07-06, en main): `cargar-dev-env.sh` aborta ruidoso si `dev.env` declara `VITE_*TOKEN*`** (prohibido hornear un Bearer en el bundle del front). `mke rama` queda como FACHADA deprecada (`packages/rama-receta/DEPRECATED.md`) que delega a `mke dev` — no hay dos mecanismos vivos.
...
- **`mke preview up|down|ls`** (2026-07-03, en main): preview por RAMA — `up <app> <rama> [--feature nombre] [--dir repo] [--dry-run]` hace git worktree efímero de la rama → `docker build` → `k3d image import -c mke-preview` → overlay generado en runtime que reusa `k8s/base` del app (namespace + postgres efímero + Secret + patches de Ingress/host + `PREVIEW=true`) → `kubectl apply -k` → CNAME `<slugApp>-<feature>-pre.mishi.com.co` → verifica alcance (reintentos). Nombre del preview = `<slugApp>-<feature>` (slug público registrado por app en `previewApps.ts`; fallback genérico si el app no está registrado). `down <nombre> [--dry-run]` borra namespace + CNAME (vía API Cloudflare; cloudflared no borra). `ls` lista namespaces vivos. Probado E2E 2026-07-03: `up mishi-bank main --feature smoke` → 200 en `/health`, `down` → namespace + CNAME limpios, cero residuo.
