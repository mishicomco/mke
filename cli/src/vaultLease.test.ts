...
test("crearLease: POST /v1/lease con ns/rama/ttlSegundos y Bearer del emisor", async () => {
  const { calls, fetchImpl } = mockFetch({
    status: 200,
    body: { leaseId: "l1", token: "tok", ns: "mishi-bank", rama: "feat-x", expiraEn: "2026-07-11T00:00:00Z" },
  });
  const r = await crearLease({ vaultUrl: "https://vault.internal", emisorToken: "emisor-tok", fetchImpl }, "mishi-bank", "feat-x", 3600);
  ...
  assert.deepEqual(JSON.parse(calls[0].init?.body as string), { ns: "mishi-bank", rama: "feat-x", ttlSegundos: 3600 });
  assert.deepEqual(r, { leaseId: "l1", token: "tok", ns: "mishi-bank", rama: "feat-x", expiraEn: "2026-07-11T00:00:00Z" });
});

test("crearLease: sin ttlSegundos, el body NO lo incluye (deja el default del vault)", async () => {
  const { calls, fetchImpl } = mockFetch({ status: 200, body: { leaseId: "l1", token: "t", ns: "a", rama: "b", expiraEn: "x" } });
  await crearLease({ vaultUrl: "https://v", emisorToken: "e", fetchImpl }, "a", "b");
  assert.deepEqual(JSON.parse(calls[0].init?.body as string), { ns: "a", rama: "b" });
});
...
test("crearLease: HTTP no-2xx revienta con el status y el cuerpo", async () => {
  ...
  await assert.rejects(
    crearLease({ vaultUrl: "https://v", emisorToken: "e", fetchImpl }, "a", "b"),
    /HTTP 403/,
  );
});
